<template>
  <div class="p-6 flex flex-col gap-6">
    ...
    <!-- Banner rimosso: il modale ServerRestartDialog (App.vue) mostra il progresso del riavvio -->
...
const { waitingRestart: restarting, captureBootId, pollUntilRestarted } = useServerRestart()
...
async function activate(name: string) {
  ...
  try {
    ...
    pollUntilRestarted(bootIdBefore, {
      ...
      onTimeout,
      dialog: {
        title: 'Attivazione plugin',
        steps: { restarting: { description: `Riavvio in corso dopo l’attivazione di «${name}»…` } },
      },
    })
  } catch {
    ...
    accept: async () => {
      ...
      try {
        ...
        pollUntilRestarted(bootIdBefore, {
          ...
          onTimeout,
          dialog: {
            title: 'Disattivazione plugin',
            steps: { restarting: { description: `Riavvio in corso dopo la disattivazione di «${name}»…` } },
          },
        })
      } catch {
        activating.value = null
