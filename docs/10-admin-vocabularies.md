# Gestione Terms (Vocabularies)

La pagina `VocabularyTermsPage.vue` gestisce i terms di un vocabulary specifico.

---

## Funzionalità

- **Lista terms** in DataTable con colonne: nome, slug, descrizione, numero di folios
- Per vocabularies **gerarchici** (es. Categories): visualizzazione ad albero con indent per livelli, campo parent nella creazione
...
- Conteggio folios associati (`postCount`) su ogni riga

---

## Navigation

Ogni vocabulary appare nella sidebar dell'admin sotto la sezione **Vocabolari**. I vocabularies sono caricati dallo store `app` tramite `GET /api/v1/vocabularies`.

---

## API Client (`src/api/vocabularies.ts`)

```ts
import { getTerms, createTerm, updateTerm, deleteTerm } from '@/api/vocabularies.js'
